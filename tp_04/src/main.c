...
int main()
{
    ...
    do{
        printf(
        		"\n1. Cargar los datos de los empleados desde el archivo data.csv (modo texto)\n2. Cargar los datos de los empleados desde el archivo data.csv (modo binario)"
        		"\n3. Alta de empleado \n4. Modificar datos de empleado \n5. Baja de empleado \n6. Listar empleados \n7. Ordenar empleados "
    			"\n8. Guardar los datos de los empleados en el archivo data.csv (modo texto).\n9. Guardar los datos de los empleados en el archivo fileBinary.bin (modo binario)."
        		"\n10. Salir\n" );
        ...
        switch(option)
        {
            ...
            case 2:
            	if (flagTxt==-1 && flagBin==-1)
            	{
            		...
            		ErrorRetorno=controller_loadFromBinary("fileBinary.bin",listaEmpleados);
            		...
            		else
            		{
            			if(ErrorRetorno==-1)
            			{
            				if(utn_PedirCualquierCaracterAUsuario(letra, 2, 0, "Presione S si desea Crear el Archivo o cualquier otra tecla para continuar", "Volviendo Al Inicio")==0 )
							{
								if(letra[0]=='s' || letra[0]=='S')
								{
									FILE* fb=fopen("fileBinary.bin", "wb");
									if(fb!=NULL)
									{
										fclose(fb);
            	...
            	break;
            case 3:
            	if(flagBin==0 || flagTxt==0 || flagAdd==0)
            	{
            	...
            	{
            		printf("Estas Seguro de Querer Agregar Sin Cargar los Archivos de Datos binario o texto\n");
            		if(utn_pedirLetrasAUsuario(letra, 2, 0, "Presione S para Desbloquear esta Opcion o cualquier otra tecla para volver al Menu Inicial", "Volviendo Al Menu")==0)
            		{
            			if(letra[0]=='s' || letra[0]=='S')
            			{
            				flagAdd=0;
            				printf("puedes Volver a ingresar un Nuevo Empleado sin Cargar Archivos Externos\n");
            			}
            		}
            ...
            case 9:
            	if(flagBin==-1)
            	{
            		...
            		if(utn_PedirCualquierCaracterAUsuario(letra, 2, 0, "Presione S si desea Guardar de Todas Maneras", "Volviendo Al Inicio")==0 )
            		{
    					if(letra[0]=='s' || letra[0]=='S')
    					{
    						if(controller_saveAsBinary("fileBinary.bin", listaEmpleados)==0)
    						{
    							printf("Guardado Con exito");
    						}
    						else
            ...
            case 10:
            	if(utn_pedirIntAUsuarioConLimites(&salir, 1, 3, 3, "Desea Guardar los Cambios Antes de Salir:\n1.-Para Guardar en Txt\n2.-Para Guardar en Binario\n3.-Para Salir sin Guardar", "Error de Seleccion")==0)
            	{
            		...
            		case 1:
            			if(flagTxt==-1)
            			{
            				printf("No Has Cargado Un Archivo Previamente, Si Existe alguno se SobreEscribira\n");
            				if(utn_PedirCualquierCaracterAUsuario(letra, 2, 0, "Presione S si desea Crear el Archivo o cualquier otra tecla para continuar", "Saliendo del Programa")==0 )
							{
								if(letra[0]=='s' || letra[0]=='S')
								{
									if(controller_saveAsText("data.csv", listaEmpleados)==0)
