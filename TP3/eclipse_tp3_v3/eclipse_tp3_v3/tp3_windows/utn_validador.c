...
static int utn_obtenerLetras(char cadena[], int len);
...
int utn_esIntPositivo(char cadena[])
{
	int retorno=-1;
	int i=0;
...
int utn_obtenerSoloNombre(char cadena[], int len)
{
	int retorno=-1;
	char buffer[len];
	fflush(stdin);
...
int utn_pedirLetrasAUsuario(char cadena[], int len, int reintentos, char* variableTexto, char* textoError)
{
	...
	if(cadena != NULL && reintentos >=0 && variableTexto != NULL && textoError != NULL && len>0)
	{
		do
		{
			printf("%s\n",variableTexto);
			fflush( stdin );
			if (utn_obtenerLetras(cadena, len)==0)
			{
				retorno = 0;
				break;
			}
...
int utn_pedirSoloNombre(char cadena[], int len, int reintentos, char* variableTexto, char* textoError)
{
	int retorno=-1;
	if(cadena != NULL && reintentos >=0 && variableTexto != NULL && textoError != NULL && len>0)
	{
		do
		{
			printf("%s\n",variableTexto);
			fflush( stdin );
			if (utn_obtenerSoloNombre(cadena, len)==0)
			{
				retorno = 0;
				break;
			}
